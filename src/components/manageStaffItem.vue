<template>
    ...
    <td class="p-2 ">
        {{ item?.HoTenNV || 'N/A' }}</td>
    <td class="p-2 ">
        {{ item?.ChucVu === 'ThuThu' ? 'Thủ Thư' : (item?.ChucVu || 'N/A') }}</td>
    <td class="p-2 flex justify-end">
        <div class="relative inline-block">
            ...
            <div 
                ...
                <ul class="py-2">
                    <li v-if="item.ChucVu !=='Admin' && item._id !== userData._id" @click="edit" class="px-4 py-2 cursor-pointer hover:bg-[#FEF6DF]">Cập nhật</li>
                    <li v-if="item.ChucVu !=='Admin' && item._id !== userData._id" @click="confirmDelete" class="px-4 py-2 cursor-pointer hover:bg-[#FEF6DF]">Xóa</li>
