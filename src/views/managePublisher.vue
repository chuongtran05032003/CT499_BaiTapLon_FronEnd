...
<router-view />
 <div
    ...
    <div class="h-fit w-full items-center pr-5">
        <h3 class="font-bold text-lg whitespace-nowrap mb-5">Quản lý nhà xuất bản</h3>
...
<script setup>
import Item from '@/components/managePublisherItem.vue';
...
const add = () => {
    router.push({ name: 'managePublisherAdd'});
}
const clearSearch = () => {
    searchInput.value = '';
...
const handleSearch = async () => {
    try {
        if (searchInput.value.trim()) {
            const response = await service.getOne(searchInput.value); // Gọi API lấy danh mục
            List.value = response;
